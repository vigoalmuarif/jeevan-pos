...
>
    <div class="dz-flex dz-flex-col dz-items-start dz-h-full dz-w-full dz-justify-center dz-bg-transparent dz-dark:border-gray-600 dz-dark:hover:border-gray-500">
        ...
        @endif
        <div @click="$refs.input.click()" class="dz-border dz-border-dashed dz-rounded dz-border-gray-500 dz-w-full rounded-xl dz-cursor-pointer">
            <div>
                <div x-show="!isDragging" class="dz-flex dz-items-center bg-slate-300/80 dark:bg-slate-800 rounded-xl dz-justify-center dz-gap-3 dz-py-8 dz-h-full">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="dz-w-4 dz-h-4 dz-md:w-6 dz-md:h-6 text-slate-600 dark:text-slate-400">
                        <path d="M9.25 13.25a.75.75 0 001.5 0V4.636l2.955 3.129a.75.75 0 001.09-1.03l-4.25-4.5a.75.75 0 00-1.09 0l-4.25 4.5a.75.75 0 101.09 1.03L9.25 4.636v8.614z" />
                        <path d="M3.5 12.75a.75.75 0 00-1.5 0v2.5A2.75 2.75 0 004.75 18h10.5A2.75 2.75 0 0018 15.25v-2.5a.75.75 0 00-1.5 0v2.5c0 .69-.56 1.25-1.25 1.25H4.75c-.69 0-1.25-.56-1.25-1.25v-2.5z" />
                    </svg>
                    <p class="dz-text-sm dz-md:text-base text-slate-600 dark:text-slate-400">Drop here or Browse files</p>
                </div>
        ...
        <div class="dz-flex dz-justify-between dz-w-full dz-mt-2">
            <div class="dz-flex dz-gap-3 dz-text-gray-500 dz-text-xs dz-md:text-sm">
                ...
                @if($hasMaxFileSize)
                    <p class="text-xs">{{ __('Up to :size', ['size' => \Illuminate\Support\Number::fileSize($this->maxFileSize * 1024)]) }}</p>
                @endif
            ...
            <div x-show="isLoading" class="dz-flex dz-gap-1 dz-items-center">
                ...
                <div @click="cancelUpload" class="dz-text-xs dz-md:text-sm text-gray-800 dark:text-gray-200 dz-hover:cursor-pointer dz-underline">Cancel upload</div>
            </div>
        </div>
        ...
        <div class="dz-flex dz-flex-wrap dz-gap-x-10 dz-gap-y-2 dz-justify-start dz-w-full dz-mt-5">
            @foreach($files as $file)
                <div class="dz-flex dz-items-center dz-justify-between dz-gap-2 dz-border dz-rounded dz-border-gray-200 dz-w-full dz-h-auto dz-overflow-hidden dz-dark:border-gray-700" id="image-list">
                    <div class="dz-flex dz-items-center dz-gap-3">
                        ...
                        @else
                            <div class="dz-flex dz-justify-center dz-items-center dz-w-14 dz-h-14 bg-gray-100 dark:bg-gray-700">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="dz-w-8 dz-h-8 text-gray-500">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                                </svg>
                            </div>
                        @endif
                        <div class="dz-flex dz-flex-col dz-items-start dz-gap-1">
                            <div class="dz-text-start dz-line-clamp-1 text-slate-900 text-xs md:text-sm font-medium dark:text-slate-100">{{ $file['name'] }}</div>
                    ...
                    <div class="dz-flex dz-items-center dz-mr-3">
                        <button type="button" @click="removeUpload('{{ $file['tmpFilename'] }}')">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="dz-w-6 dz-h-6 text-rose-500">
